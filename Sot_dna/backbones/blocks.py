...
class ConvBlock(nn.Module):
    ...
    def __init__(self, indim, outdim, convdim=1, pool=True, padding=1):
        super(ConvBlock, self).__init__()
        self.indim = indim
        self.outdim = outdim
        if self.maml:
            self.C = Conv1d_fw(indim, outdim, 1, padding=padding)#here 123123
...
class SimpleBlock(nn.Module):
    ...
    def __init__(self, indim, outdim, half_res):
        ...
        self.outdim = outdim
        if self.maml:
            self.C1 = Conv1d_fw(indim, outdim, kernel_size=3, stride=2 if half_res else 1, padding=1, bias=False)
            self.BN1 = BatchNorm1d_fw(outdim)
            self.C2 = Conv1d_fw(outdim, outdim, kernel_size=3, padding=1, bias=False)
            self.BN2 = BatchNorm1d_fw(outdim)
        else:
            self.C1 = nn.Conv1d(indim, outdim, kernel_size=3, stride=2 if half_res else 1, padding=1, bias=False)
            self.BN1 = nn.BatchNorm1d(outdim)
            self.C2 = nn.Conv1d(outdim, outdim, kernel_size=3, padding=1, bias=False)
        ...
        if indim != outdim:
            if self.maml:
                self.shortcut = Conv1d_fw(indim, outdim, 1, 2 if half_res else 1, bias=False)
                self.BNshortcut = BatchNorm1d_fw(outdim)
            else:
                self.shortcut = nn.Conv1d(indim, outdim, 1, 2 if half_res else 1, bias=False)
