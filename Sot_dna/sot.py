...
class SOT(MetaTemplate):
    def __init__(self, backbone, n_way, n_support, n_task, task_update_num, inner_lr, approx=False):
        super(SOT, self).__init__(backbone, n_way, n_support, change_way=False)
    ...
    def set_forward(self, x, y=None):
        ...
        support_feature = self.sot(self.forward(x_a_i))
        query_feature = self.sot(self.forward(x_b_i))
        
        support_score = self.classifier(support_feature)
        query_score = self.classifier(query_feature)
