...
class TMSetDataset(TMDataset):
    ...
    def __init__(self, n_way, n_support, n_query, n_episode=100, root='./data', mode='train'):
        ...
        # print("samples_allshape=",samples_all.shape)
...
# class TMSimpleDatasetConv(TMSimpleDataset):
#     def __init__(self, batch_size, root='./data/', mode='train', min_samples=20):
#         super().__init__(batch_size, root, mode, min_samples)

#     def reshape_and_normalize(self, sample):
#         height_length = 2
#         width_length = 1433
#         reshaped_sample = np.reshape(sample, (1, height_length, width_length))
#         normalized_sample = (reshaped_sample - np.mean(reshaped_sample)) / np.std(reshaped_sample)
#         return normalized_sample

#     def __getitem__(self, i):
#         sample = self.samples[i]
#         target = self.targets[i]
#         sample = self.reshape_and_normalize(sample)
...
# class TMSetDatasetConv(TMSetDataset):
#     def __init__(self, n_way, n_support, n_query, n_episode=100, root='./data', mode='train'):
#         super().__init__(n_way, n_support, n_query, n_episode, root, mode)
